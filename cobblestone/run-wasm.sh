#!/usr/bin/env bash
# Compile each unit's IR (from codexir) to a wasm module with the wasm plug,
# run it, and compare its output to the co-located .expected file.
#
#   ./run-wasm.sh
#
# The IR comes from upstream's frontend, so this tests the plug, not the
# frontend.
set -uo pipefail
